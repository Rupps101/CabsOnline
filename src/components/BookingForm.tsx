...
function BookingForm() {
  const [form, set_form] = useState<FormData>({
    ...
    date: get_today(), time: get_time_now()
  })

  const [errors, set_errors]             = useState<Partial<FormData>>({})
  const [error_banner, set_error_banner] = useState('')
  const [map_state, set_map_state]       = useState<MapState>({ visible: false, latitude: -36.8485, longitude: 174.7633, address: '' })
  ...
  const confirmation_ref = useRef<HTMLDivElement | null>(null)

  useEffect(() => {
    if (confirmation && confirmation_ref.current) {
      confirmation_ref.current.scrollIntoView({ behavior: 'smooth', block: 'start' })
    }
  }, [confirmation])
  ...
  const validate = (): boolean => {
    ...
    else if (!/^\d{10,12}$/.test(form.phone.trim())) new_errors.phone = 'Phone must be 10-12 digits.'
    ...
    else if (!/^\d{2}\/\d{2}\/\d{4}$/.test(form.date.trim())) new_errors.date = 'Date must be DD/MM/YYYY.'
    if (!form.time.trim())    new_errors.time    = 'Pickup time is required.'
    else if (!/^([01]\d|2[0-3]):[0-5]\d$/.test(form.time.trim())) new_errors.time = 'Time must be HH:MM.'
  ...
  const show_map = async () => {
    if (!form.snumber.trim() || !form.stname.trim()) {
      set_error_banner('Enter a street number and street name to show the map.')
      return
    }
    set_error_banner('')
    set_geocoding(true)
    ...
    try {
      ...
      if (data.length > 0) {
        ...
        set_map_state({ visible: true, latitude: -36.8485, longitude: 174.7633, address: query + ' (approximate)' })
      }
    } catch {
      set_map_state({ visible: true, latitude: -36.8485, longitude: 174.7633, address: query + ' (approximate)' })
    }
  ...
  const handle_payment_success = async () => {
    ...
    try {
      const res    = await fetch(API + '/booking.php', { method: 'POST', body: fd, mode: 'cors', credentials: 'omit' })
      const result = await res.json()

      if (result.success) {
        set_confirmation(result)
        set_form({ cname: '', phone: '', unumber: '', snumber: '', stname: '', sbname: '', dsbname: '', date: get_today(), time: get_time_now() })
        set_map_state({ visible: false, latitude: -36.8485, longitude: 174.7633, address: '' })
      } else {
  ...
  return (
    <div>
      <div className="page-intro">
        <h1>Book a Ride</h1>
        <p>Fill in your details, preview your pickup on the map, then confirm your booking.</p>
      </div>

      {confirmation && (
        <div ref={confirmation_ref} className="success-banner">
          ✅ Thank you for your booking!<br />
          Booking reference number: <strong>{confirmation.booking_ref}</strong><br />
          Pickup time: <strong>{confirmation.pickup_time}</strong><br />
        ...
        <form onSubmit={handle_submit} noValidate>
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <input name="cname" value={form.cname} onChange={handle_change} placeholder="e.g. Jane Smith" className={errors.cname ? 'input-error' : ''} />
              {errors.cname && <span className="field-error">{errors.cname}</span>}
            </div>
            <div className="form-group">
              <label>Phone Number <span className="required">*</span></label>
              <input name="phone" value={form.phone} onChange={handle_change} placeholder="e.g. 0211234567" maxLength={12} className={errors.phone ? 'input-error' : ''} />
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <input name="unumber" value={form.unumber} onChange={handle_change} placeholder="e.g. 4B" />
              <span className="field-hint">Optional</span>
            </div>
            <div className="form-group">
              <label>Street Number <span className="required">*</span></label>
              <input name="snumber" value={form.snumber} onChange={handle_change} placeholder="e.g. 42" className={errors.snumber ? 'input-error' : ''} />
              {errors.snumber && <span className="field-error">{errors.snumber}</span>}
            </div>
          </div>
          <div className="form-group">
            <label>Street Name <span className="required">*</span></label>
            <input name="stname" value={form.stname} onChange={handle_change} placeholder="e.g. Queen Street" className={errors.stname ? 'input-error' : ''} />
            {errors.stname && <span className="field-error">{errors.stname}</span>}
          </div>
          <div className="form-row">
            <div className="form-group">
              <label>Pickup Suburb</label>
              <input name="sbname" value={form.sbname} onChange={handle_change} placeholder="e.g. Auckland CBD" />
              <span className="field-hint">Optional</span>
            </div>
            <div className="form-group">
              <label>Destination Suburb</label>
              <input name="dsbname" value={form.dsbname} onChange={handle_change} placeholder="e.g. Newmarket" />
              <span className="field-hint">Optional</span>
            </div>
          </div>

          <button type="button" className="btn-secondary" onClick={show_map} disabled={geocoding} style={{ marginBottom: '8px' }}>
          ...
          <p className="section-title" style={{ marginTop: '24px' }}>Pickup Date &amp; Time</p>
          <div className="form-row">
            <div className="form-group">
              <label>Pickup Date <span className="required">*</span></label>
              <input name="date" value={form.date} onChange={handle_change} placeholder="dd/mm/yyyy" className={errors.date ? 'input-error' : ''} />
              {errors.date && <span className="field-error">{errors.date}</span>}
              <span className="field-hint">Format: DD/MM/YYYY</span>
            </div>
            <div className="form-group">
              <label>Pickup Time <span className="required">*</span></label>
              <input name="time" value={form.time} onChange={handle_change} placeholder="e.g. 17:30" className={errors.time ? 'input-error' : ''} />
